...
// ── Raw data (paste-refreshable monthly) ──────────────────────────────────────
const RAW_DATA = [
  {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: "0 0 6px", fontSize: 22, fontWeight: 700, color: "#1c3a54", letterSpacing: "-0.02em" }}>
          Electrolux Global<br/>Design Projects
        </h1>
        <p style={{ margin: "0 0 28px", fontSize: 12, color: "#6b7c88" }}>Enter your access password to continue</p>
        <input
          type="password"
          value={pw}
          ...
          style={{
            ...
            padding: "10px 14px",
            fontSize: 14,
            border: `1.5px solid ${error ? "#c0392b" : "#cdd3d8"}`,
            borderRadius: 8,
            outline: "none",
        ...
        />
        {error && (
          <div style={{ color: "#c0392b", fontSize: 11, marginBottom: 8 }}>Incorrect password. Please try again.</div>
        )}
        <button
          onClick={attempt}
          style={{
...
function GanttChart() {
  const globalMin = RAW_DATA.flatMap(p => p.phases.map(ph => ph.start)).sort()[0];
  const globalMax = RAW_DATA.flatMap(p => p.phases.map(ph => ph.end)).sort().reverse()[0];
  ...
  const [rangeStart, setRangeStart] = useState(globalMin);
  const [rangeEnd, setRangeEnd] = useState(globalMax);
  ...
  const allRoadmaps = ["All", "Identity", "Implementation", "Innovation"];
  ...
  }, [data, filterClients, filterStatuses, filterLocations, filterRoadmaps]);
  ...
  const todayOffset = clamp(daysBetween(rangeStart, today) / totalDays, 0, 1);
    ...
    <div style={{
      ...
      {csvMode && (
        <div style={{
          ...
          <div style={{ fontSize: 11, color: "#6b7c88", marginBottom: 14, fontFamily: "monospace" }}>
            Project, Client, Status, Roadmap, Locations, Phase Name, Start Date, End Date, Hours
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, color: "#9aabb5" }}>The chart will update automatically on upload.</span>
          </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #dde1e4" }}>
          <div style={{ width: 220, minWidth: 220, padding: "10px 16px", fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: "#6b7c88", textTransform: "uppercase", borderRight: "1px solid #dde1e4" }}>
            Project / Client
          </div>
        ...
        {filtered.map((proj, ri) => {
          ...
          return (
            ...
            >
              <div style={{
                width: 220, minWidth: 220, padding: "8px 12px 8px 14px",
